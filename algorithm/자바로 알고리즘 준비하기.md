...
KMP 알고리즘은 문자열에서 특정 문자 패턴을 찾는 알고리즘이다.

문자 패턴을 왼쪽부터 하나씩 비교해나가는데, 이 때 현재까지 비교한 내용을 바탕으로 불필요한 비교를 최대한 스킵한다.

이를 위해 문자 패턴에 대하여 접두사 == 접미사를 만족하는 가장 긴 부분문자열의 길이를 구한다.

```
X = ABABABAC

x[i] = 0~i로 구성된 부분문자열에 대하여 접두사 == 접미사를 만족하는 가장 긴 부분문자열의 길이. (단, 부분문자열 그 자체는 제외)

e.g) x[0] = 0, x[1] = 0, x[2] = 1, x[3] = 2(AB), x[4] = 3(ABA), x[5] = 4(ABAB), x[6] = 5(ABABA), x[7] = 0
```

"접두사 == 접미사"를 만족한다는 내용이 중요하다.
X = X0X1X2X3라는 문자 패턴을 인풋 문자열과 비교할 때, X0 부터 비교하기 시작한다.
X0X1X2까지 일치했다고 가정해보자.
X3은 불일치하기 때문에 이제 문자패턴을 이동시켜야 한다.
naive한 방법은 단순히 오른쪽으로 1만큼 이동시키는 것이다.
그런데 X0과 X1과 다르다면 오른쪽으로 1만큼 이동하여도 어차피 불일치한다.
여기서 우리가 구한 `x[i]`를 사용한다.

`불일치한 인덱스 - x[i] (i는 x0x1x2이므로 여기서는 2이다.)` 로 X를 이동시킨다.

```
문자열은 접미사와 일치하는 접두사의 위치로 이동한다.
[[  ] [  ]]
     [[  ]  [  ]]
```

이동한 뒤에도 앞서 불일치한 위치의 문자값과 일치하지 않을 수 있다.
그래서 이 이동 과정은 문자값과 일치할 때까지 반복한다.

문자패턴의 길이만큼 이동해도 문자값과 일치하지 않는다면 그 문자값은 문자패턴에 없는 것이다.
그래서 다음 문자값으로 넘어간다.
...
KMP 알고리즘은 패턴에 대한 인덱스와 문자열에 대한 인덱스를 따로 관리하면서 서로 한 문자씩 비교해나간다.

효율적으로 비교하기 위해 pi 배열을 구해야 한다.
pi 배열을 구하고, 이용하는 것이 KMP 알고리즘의 핵심이다.

`pi[i] = i까지의 접두사 == 접미사를 만족하는 가장 긴 부분문자열의 길이이다. (단, 부분문자열 그 자체는 제외이다.)`

어떤 문자 패턴이 문자열과 i-1까지 일치하고, i에서 불일치할 때 패턴을 가리키는 인덱스는 `pi[i-1]`로 이동한다.
pi 배열은 패턴을 패턴으로 문자열 체크를 하여 만들어지기 때문에, 이미 일치, 불일치 여부를 알고 있는 것으로 볼 수 있다.

i-1까지 일치했다면 문자열은 i-1까지 패턴과 동일하게 취급될 수 있다.
그런데 pi 배열을 통해 이미 패턴에 대한 분석이 끝났으므로 일치할 가능성이 있는 다음 위치로 이동할 수 있다

### 구현

구현은 pi 배열을 구하는 과정과 패턴 매칭 부분으로 나뉜다.

pi 배열을 구하는 과정은 패턴을 패턴으로 분석해야 한다.
`pi[0]`은 언제나 0이므로(부분문자열 자체는 제외하므로), `i=1`부터 시작한다.
...
```java
    static class KMP {
        ...
        int find(String t, List<Integer> founds) {
            int j = 0;
            int count = 0;
            int pn = p.length();
            int n = t.length();
...
## 맨버-마이어스 알고리즘 (접미사배열 만들기)

접미사 배열은 문자열의 모든 접미사를 사전순서로 정렬한 배열이다.

맨버-마이어스 알고리즘은 접미사 배열을 만드는 방법 중 하나이다.

시간복잡도는 O(N*(logn)^2)이다.

맨버-마이어스 알고리즘의 핵심은 접미사를 그룹화하여 정렬하는 것이다.
접미사를 그룹화하는 이유는 접미사의 특징을 활용하여 시간복잡도를 줄이기 위함이다.

접미사의 특징은 다음과 같다.
"모든 접미사에 대하여 접미사 X의 부분문자열이 접두사인 접미사 Y가 존재한다."
```
P = X0X1...Xn
X = XaXa+1Xa+2Xa+3...Xn (0 <= a <= n)

이 때, X의 부분문자열이 접두사인 접미사 Y가 반드시 존재한다.

예를 들어, X의 부분문자열 Xa+2Xa+3이 접두사인 접미사 Y는 아래와 같이 존재한다.
Y = Xa+2Xa+3...Xn
```

맨버-마이어스 알고리즘은 접미사를 앞에서부터 한글자씩 비교하면서 그룹핑을 한다. 
비교 결과에 따라 랭킹을 매기고, 랭킹 순위에 따라 그룹번호가 정해진다.
이 때, 접미사 문자열의 비교 인덱스를 0부터 시작하여 1씩 증가하며 비교하는 게 아니라, 2의 승수만큼 증가하여 비교한다.
즉,  t = 0 -> 1 -> 2 -> 4 -> 8 -> 16 ... 이다.

이렇게 할 수 있는 이유는 접미사의 특징을 활용하여 기존에 비교했던 내용을 가지고 비교하기 때문이다.

```c++
i, j에 들어올 인자는 suffix i와 suffix j다 
bool comp(int i, int j) {
	if (group[i] != group[j]) // 만약 접미사 i와 접미사 j의 그룹이 같지않다면,
		return group[i] < group[j]; // 접미사 j의 그룹번호가 접미사 i의 것보다 더 큰가?? 
    //만약 두 접미사의 그룹이 같다
	return group[i + t] < group[j + t]; // 접미사 i + t의 그룹번호가 접미사 j + t의 그룹번호보다 큰가?
}
```
출처: https://m.blog.naver.com/zxwnstn/221573132076?recommendTrackingCode=2

`comp`함수는 그룹핑시 사용되는 비교함수이다. 랭킹을 결정해준다.
이 함수에서 중요한 지점은 `t`의 사용이다.
`t`는 비교 인덱스로서, 접미사들을 왼쪽부터 `t`까지 비교하여 그룹핑을 해놓은 상황이다.

`group[i+t] < group[j+t]`의 의미는 각 접미사의 t부터 t*2까지를 비교하는 것이다.
예를 들어, X0X1과 X10X11을 비교할 때, `group[i+t] < group[j+t]`는 t가 2라면 i+t는 X2X3을 의미하고, j+t는 X12X13을 의미한다.
결국 `comp`함수는 X0X1과 X10X11을 비교한 뒤 X2X3과 X12X13을 비교하므로 X0X1X2X3과 X10X11X12X13을 비교하는 셈이다.
다시 말해 t가 2일 때 왼쪽부터 4만큼의 문자열을 비교한다.
때문에 t를 2의 승수만큼 증가해도 되는 것이다.

이러한 특징 덕분에 전체 비교 횟수는 logN이 된다.


### 구현

그룹핑은 결국 문자 비교를 통해 사전순으로 정렬하는 것이다.
그래서 그룹핑은 접미사들을 정렬하고, 정렬된 접미사들을 바탕으로 기존 그룹을 새 그룹으로 덮어쓴다.

`그룹핑: 접미사 정렬 -> 새 그룹으로 replace`

그리고 매 iteration 마다 접미사 배열을 정렬하는데, 이는 기존 랭킹 정보와 새 t 값을 활용하여 동일한 랭킹을 갖는 접미사들의 랭킹을 나눈다.
기존의 동일한 랭킹을 갖는 접미사들은 새 t 값으로 비교하여 랭킹을 정한다.
이후 이 갱신된 접미사 배열을 통해(갱신된 랭킹) 새로운 그룹을 만든다.
갱신된 랭킹에서 랭킹이 나뉜 부분과 여전히 동일한 랭킹을 갖는 부분을 반영하는 것이다.

`sort(동일 랭킹 나누기) -> 새 그룹 replace(갱신된 랭킹 반영) -> sort(동일 랭킹 나누기) -> ...`
    ...
    static class SuffixArray {
        ...
        List<Integer> from(String s) {
            ...
            while(t < n) {
                ...
                for (int i = 1; i < n; i++) {
                    int suffix1 = sa.get(i-1);
                    int suffix2 = sa.get(i);
                    if (comp(suffix1, suffix2) < 0) {
                        ng[suffix2] = ng[suffix1] + 1;
                    } else {
                        ng[suffix2] = ng[suffix1];
                    }
                }
                group = ng;
                t <<= 1;
...
참고: https://m.blog.naver.com/zxwnstn/221573132076?recommendTrackingCode=2

## LCP

## 레이지 프로파게이션

구간 업데이트를 처리하는 세그먼트 트리이다.

원래 세그먼트 트리는 구간 쿼리와 요소 업데이트만을 처리한다.
레이지 프로파게이션을 적용하면 구간 내의 여러 요소를 동일한 값만큼 업데이트하는 "구간 업데이트" 기능을 지원할 수 있다.

레이지 프로파게이션은 최대한 업데이트를 미룬다는 철학이다.
특정 구간의 업데이트를 최대한 미루고, 그 구간을 포함하는 조회 쿼리나 추가적인 업데이트 요청이 들어올 때 해당 구간에 대해 업데이트한다.

이를 위해 lz 배열을 추가한다.
lz 배열은 해당 노드의 서브트리에 대하여 업데이트 예정인 값을 저장한다.
다시 말해, 서브트리에 대한 업데이트를 미루고 이후의 조회 쿼리나 업데이트 요청이 들어오면 서브트리에 반영한다.
...
```java
 static class SegTree {
        ...
        public SegTree(long[] a) {
            this.n = a.length - 1;
            int h = (int)Math.ceil(Math.log(n) / Math.log(2));
            int n = (int)Math.ceil(Math.pow(2, h+1)) + 1;
        ...
        long init(int s, int e, int node, long[] a) {
            if (s == e) {
                return nodes[node] = a[e];
            }
            int mid = (s + e) / 2;
            return nodes[node] = init(s, mid, node*2, a) + init(mid+1, e, node*2+1, a);
        }

        void update(int l, int r, long diff) {
            _range_update(1, n, l, r, 1, diff);
        }

        void _range_update(int s, int e, int l, int r, int node, long diff) {
            _push(s, e, node);
            if (s > r || e < l) {
                return;
            }
            if (l <= s && e <= r) {
                lz[node] += diff;
		//이 노드의 부모를 포함하는 조회 쿼리에 대응하려면 이 노드는 업데이트를 미루면 안 된다.
		//즉시 업데이트하여 부모에도 업데이트 내용을 반영한다.
                _push(s, e, node);
                return;
            }
            int mid = (s + e) / 2;
            _range_update(s, mid, l, r, node*2, diff);
            _range_update(mid+1, e, l, r, node*2+1, diff);
            nodes[node] = nodes[node*2] + nodes[node*2+1];
        }

        void _push(int s, int e, int node) {
            if (lz[node] != 0) {
                nodes[node] += (e-s+1)*lz[node];
                if (s != e) {
                    lz[node*2] += lz[node];
                    lz[node*2+1] += lz[node];
                }
        ...
        long query(int l, int r) {
            return _query(1, this.n, l, r, 1);
        }

        long _query(int s, int e, int l, int r, int node) {
            _push(s, e, node);
            ...
            int mid = (s+e)/2;
            return _query(s, mid, l, r, node*2) + _query(mid+1, e, l, r, node*2+1);
        }
    }
```
...
DFS를 이용하여 그래프에서 스패닝 트리를 찾는 알고리즘이다.

<MessageBox title='스패닝 트리' level='info'>
	스패닝 트리란 그래프 내 모든 정점을 포함하는 트리이다.
</MessageBox>

그런데 이 때, 간선의 종류를 나눈다.
간선의 종류를 나누어 이후의 알고리즘에서 활용한다.

간선의 종류는 네 가지이다.

- 트리 간선: 트리가 포함하는 간선. (u, v)에서 v가 u보다 항상 늦게 발견된다.
- 역방향 간선: 자손이 조상으로 이어지는 간선. (u, v)에서 v가 u보다 항상 먼저 발견된다.
- 순방향 간선: 조상이 자손으로 이어지는 간선. (u, v)에서 v가 u보다 항상 늦게 발견된다.
- 교차 간선: 트리에 포함되지 않으면서도 자손과 조상 관계가 아닌 간선. (u, v)에서 v가 u보다 항상 먼저 발견된다. (u가 먼저 방문되었다면 (u, v)는 트리 간선이 된다.)

역방향 간선과 교차 간선은 해당 노드에서 dfs가 종료되었는지로 구분된다.
(u, v)에서 역방향 간선의 경우 v는 아직 dfs가 종료되지 않았으며, 교차 간선의 경우 dfs가 종료되었다.


```java
    static void dfs(int here) {
        d[here] = ++count;
        for (Integer o : g.get(here)) {
            if (d[o] == 0) {
                //트리 간선
                dfs(o);
            } else {
...
절단점이란 정점을 제거했을 때 커넥티드 컴포넌트가 증가하는 정점을 말한다.
커넥티드 컴포넌트란 그냥 그래프라고 보면 된다.
한 개의 그래프에서 절단점을 제거하면 두 개 이상의 그래프가 생긴다.

정점 u의 자손에서 역방향 간선이 존재하지 않으면 정점 u는 절단점이다.
그래서 절단점을 찾으려면 dfs를 통해 그래프를 순회하면서 자손에서 역방향 그래프를 보이는 정점을 찾으면 된다. (절단점은 여러 개일 수 있다.)
...
static int findCutVertex(int here, boolean isRoot) {
        ...
        for (Integer o : g.get(here)) {
            if (d[o] > 0) {
                ret = Math.min(ret, d[o]);
            } else {
                children += 1;
                int subtree = findCutVertex(o, false);
                if (!isRoot && subtree >= d[here]) {
                    cv[here] = true;
                }
...
타잔 알고리즘은 그래프에서 SCC를 찾는 알고리즘이다.

방향그래프에서 두 정점이 양방향으로 도달하는 경로가 있으면, 그 두 정점은 같은 SCC(strongly connected components)에 속한다.

SCC를 단순히 싸이클로 이해해도 좋다. 다만, SCC는 정점이 한 개인 경우도 존재한다.

![image](https://github.com/user-attachments/assets/6a6e038a-3b5a-48be-82be-969dec915132)
출처: https://nsgg.tistory.com/126

SCC에서는 서브트리에서 역방향 간선이 존재하지 않아야 하며, 교차 간선 또한 제외되어야 한다.
이것이 타잔 알고리즘에서 SCC를 찾는 핵심 요구 사항이다.

이를 만족시키려면 두 개의 배열이 필요하다. discovered와 sccId 이다.
discovered를 통해 노드 방문시 카운팅한다. 카운팅값은 노드 방문 순서이며, 이 값으로 역방향 간선 발생 유무를 파악할 수 있다.
교차 간선을 제외하려면 이미 scc로 그룹핑된 노드를 방문하는 경우인지를 체크해야 한다. 이 때 사용되는 것인 sccId이다.
scc를 만들 떄 sccId에 카운팅한다.
...
타잔 알고리즘와 같이 그래프에서 SCC를 찾는 알고리즘이다.

핵심 아이디어는 각 SCC 루트노드(이하 대표노드)의 특성과 관련이 있다.

대표노드는 가장 나중에 dfs 탐색이 완료된다.
대표노드는 가장 처음에 방문되지만, 재귀함수를 통해 다음 노드들을 차례로 방문을 하고 각 다음 노드에 대한 함수가 모두 리턴된 후 리턴된다.
트리에서 후위 순회와 동일하다.

SCC 간에도 먼저 탐색된 SCC가 나중에 탐색이 완료된다.
SCC를 하나의 정점으로 보는 그래프 압축을 통해 DAG가 만들어지고, DAG에는 항상 시작점(즉, 시작 SCC)이 있다.

일단, 탐색 완료된 노드를 스택에 저장해보자.
가장 위에 있는 노드는 시작 SCC의 대표노드이다. 이어서 시작 SCC의 노드들이 그 다음에 위치하고, 다음 SCC의 대표노드가 나타날 것이다.

순서적으로는 SCC를 스택에 정리한 셈이다. 문제는 SCC 간에 연결되는 구간을 어떻게 찾을 것이냐이다.

코사라주 알고리즘은 "역방향 그래프"로 이 문제를 해결한다.
역방향 그래프란 기존 그래프의 모든 간선의 방향을 반대로 돌린 그래프이다.
역방향 그래프에서 그래프 압축을 하면 기존 DAG와 반대방향으로 간선이 구성된다.
따라서 스택에 저장된 순서대로 dfs를 역방향 그래프에서 다시 실행하면, 타 SCC로 넘어가지 않고 하나의 SCC에서 탐색이 종료된다. (SCC 밖으로 향하는 간선을 반대방향으로 뒤집어버렸기 때문에)
...
오일러 회로 문제를 푸는 알고리즘이다.

오일러 회로 문제는 그래프에서 모든 간선을 한번씩 지나면서 모든 정점을 방문할 수 있는 경로를 구하는 문제이다.
경로가 존재하려면 다음의 조건을 만족해야 한다.

- 그래프는 연결되어 있어야 한다. (하나의 연결 컴포넌트)
- 모든 정점의 간선은 짝수여야 한다.

경로를 구하려면 정점을 방문하면서 지나온 간선을 제거하면 된다.
...
힙인데, 힙의 원소를 O(1)만에 접근하여 변경하는 자료구조이다.

인덱스드힙은 바깥에서 인덱스-밸류 매핑을 알고 있다는 전제 하에 설계된 자료구조이다.
인덱스드힙은 호출자에서 인덱스를 통해 특정 노드를 갱신하거나 삭제할 수 있도록 제공해야 한다.

이를 위해 인덱스드힙 안에서 인덱스와 노드의 위치는 양방향으로 추적 가능해야 한다.
호출자에서 인덱스를 통해 특정 노드를 갱신하거나 삭제하므로 인덱스를 통해 노드를 추적할 수 있어야 하며,
힙의 구조를 유지하는 과정에서 노드의 위치가 변경이 될 때 인덱스 또한 변경된 위치로 반영되어야 하므로 노드의 위치를 통해 인덱스를 알아낼 수 있어야 하기 때문이다.
        ...
        void down(int node) {
            while((node << 1) <= c) {
                if ((node << 1) == c) {
                    ...
                    int lch = node << 1;
                    int rch = (node << 1) + 1;
                    if (Math.min(h[lch].v, h[rch].v) >= h[node].v) {
                        break;
                    }
...
참고: https://infossm.github.io/blog/2020/08/16/heap/
